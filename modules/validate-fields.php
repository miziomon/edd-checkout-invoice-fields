...
add_action('edd_checkout_error_checks', function ( $valid_data, $data ) {
    ...
    if (empty($data['edd_address'])) {
        edd_set_error('invalid_address', __('Please enter valid Address.', 'edd_cif'));
    }
    
    if (empty($data['edd_company'])) {
        edd_set_error('invalid_company', __('Please enter valid Company Name.', 'edd_cif'));
    } 
    
    if (empty($data['edd_fc']) ) {
        edd_set_error('invalid_fc', __('Please enter valid Fiscal code.', 'edd_cif'));
    } 

    
    $vat_validation = new VatValidator(  $data['edd_vat'] );
    if ( !$vat_validation ) { 
        edd_set_error('invalid_vat', __('Please enter valid VAT', 'edd_cif'));
    }
